...
const ShimmerBlock = ({ className }: { className: string }) => {
  return (
    <div
      className={`relative overflow-hidden bg-gray-900/50 rounded-lg ${className}`}
    >
    ...
    <div className="w-full min-h-screen space-y-6 p-6 bg-[#0A0A0A] border border-gray-900/50 shadow-2xl shadow-black/50">
      ...
      <div className="space-y-3">
        {[1, 2, 3, 4, 5].map((item) => (
          <div
            key={item}
            className="flex items-center p-4 rounded-xl bg-gray-900/20 border border-gray-800/50"
          >
            {/* ID e Avatar */}
