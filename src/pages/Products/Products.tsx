...
const Products = () => {
  ...
  const [data, setData] = useState<RootProductResponse>({ product: [], totalDoc: 0 })
  const [page, setPage] = useState(1)
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState("");

  useEffect(() => {
    axios.post(`http://localhost:3000/data/product?page=${page}&search=${search}`, {
  ...
  const metadata = data.product[0]?.metadata[0] || { total: 0, totalQuantity: 0, totalPrice: 0 };
    ...
    <div className="p-8 bg-gray-950 w-full min-h-screen text-gray-100">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div className="bg-gray-900/40 border border-gray-800 p-6 rounded-2xl">
          ...
          <h3 className={`text-3xl font-bold mt-1 ${true ? 'text-amber-500' : 'text-green-400'}`}>{metadata.totalQuantity}</h3>
        </div>
        <div className="bg-gray-900/40 border border-gray-800 p-6 rounded-2xl">
          <p className="text-gray-500 text-sm">Valor em Estoque</p>
          <h3 className="text-3xl font-bold mt-1 text-blue-500">{formatPrice(metadata.total)}</h3>
        </div>
      </div>
        ...
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-gray-800">
             {products.map((product: any) => (
              <tr className="hover:bg-gray-800/30 transition-colors group" key={product._id}>
                <td className="px-6 py-4">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-gray-800 rounded-lg overflow-hidden border border-gray-700">
                      <img src="https://via.placeholder.com/150" className="w-full h-full object-cover" alt="Produto" />
                    </div>
                    <div>
                ...
                <td className="px-6 py-4 text-sm text-gray-400">{product.categorie}</td>
                <td className="px-6 py-4 font-medium text-gray-200">{formatPrice(product.price)}</td>
                <td className="px-6 py-4">
                  <div className="flex items-center gap-2">
                    <span className={`w-2 h-2 rounded-full ${product.quantity > 10 ? 'bg-green-500' : 'bg-amber-500'}`}></span>
                    <span className="text-sm text-gray-300">{product.quantity} unidades</span>
                  </div>
                </td>
                <td className="px-6 py-4 text-center">
                  <button className="p-2 hover:bg-gray-700 rounded-lg text-gray-500 transition-colors">
